  ...
  function ajaxCall(mode, aplace, index){
       ...
       switch(mode) {
        ...
        case 3: params = aplace + '&mode=3&parameters='+index;
                break;
        //remove
        case 1: if(document.getElementById('input' + index).value == 1){
...
var mode;
 function ajaxAddProduct(plac, index, not, productId, mod){
  ...
  function showNotifications(index, not) {
      document.getElementById('notification').innerHTML = not + ' foi adicionado com sucesso.';  
      if(row.value == null) {
           row.value = 1;
           row.innerHTML = 1;
      } else {
...
function confirmDeleting(floor, place) {
    if(confirm('Você tem certeza que deseja deletar essa mesa?'))
...
function getIndexBackByEditing(nr, place){
    ...
    if(isNaN(value)){
        alert('Por favor, entre com um numero válido!');
        params = place + '&mode=0';
        reloadTheWindowIfRequired();
    } else if (value > 0){
